...
class FlatMapInfo(object):
    ...
    def d_grade(self, mp, x_fac, y_fac = None):
        ...
        mp_dg = np.mean(np.mean(np.reshape(mp2d.flatten(), [ny_new, int(y_fac), nx_new, int(x_fac)]), axis = -1),
                        axis = -2).flatten()
    ...
    def smooth_map(self, map_in, larr, beam):
        ...
        return np.fft.irfft2(beam_map * np.fft.rfft2(map_in.reshape([self.ny, self.nx])),
                             s = [self.ny, self.nx]).flatten()
